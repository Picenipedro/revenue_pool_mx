...
st.set_page_config(page_title="Custom Streamlit App",
                   layout="wide",
                   initial_sidebar_state="expanded")

with st.sidebar:
    st.image("bbva.png", width=150) #    
    st.title("CIB Estimated Financial Services Wallet")
    
    # Selectbox for Company
    company_list = df["Company"].unique()
    selected_company = st.selectbox("Empresa", ["None"] + list(company_list))
    
    # Selectbox for Sector
    sector_list = df_sector_payments["sector"].unique()
    selected_sector = st.selectbox("Sector", ["None"] + list(sector_list))
    
    # Multiselect for Year
    year_list = df["Year"].unique()
    selected_years = st.multiselect("Año", year_list, default=year_list)

    color_theme_list = ['blues', 'cividis', 'greens', 'reds', 'rainbow',
                        'inferno', 'magma', 'plasma', 'turbo', 'viridis']
...
selected_color = color_map.get(selected_color_theme, '#000000')  # Default to black if theme not found
...
with col1:
    ...
    <div style='font-size:14px;'>
        <p style='margin:4px 0;'>Ingresos Netos de Ventas: 
            <span style='color:{}; font-size:18px; font-weight:bold;'>${:,.2f}</span>
        </p>
        <p style='margin:4px 0;'>Activos Financieros a Corto Plazo: 
            <span style='color:{}; font-size:18px; font-weight:bold;'>${:,.2f}</span>
        </p>
        <p style='margin:4px 0;'>Efectivo y Equivalentes: 
            <span style='color:{}; font-size:18px; font-weight:bold;'>${:,.2f}</span>
        </p>
        <p style='margin:4px 0;'>Gastos de venta y distribución: 
            <span style='color:{}; font-size:18px; font-weight:bold;'>${:,.2f}</span>
        </p>
        <p style='margin:4px 0;'>Costo de Mercancía Vendida: 
            <span style='color:{}; font-size:18px; font-weight:bold;'>${:,.2f}</span>
        </p>
    </div>
    """.format(
        selected_color, total_ingresos_netos_ventas,
        selected_color, total_activos_financieros_corto_plazo,
        selected_color, total_efectivo_equivalentes,
        selected_color, total_gastos_venta_distribucion,
        selected_color, total_costo_mercancia_vendida
    ...
    st.markdown("#### Porcentaje de recaudo")
    ...
    st.markdown("#### Porcentaje de Pagos")
...
with col2:
    ...
    st.markdown("## Revenue Pool Total por Año")
